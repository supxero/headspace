...
- The sticky note's sizes are measured limits, not taste, and must not be flattened to one number. The corner is 232px wide from 901 to 1199px and 464px above that, and `#main.stickycorner #board`'s `padding-right` must always be at least the corner width plus its 22px offset: that reservation is what clears the last column at max scroll AND what stops a non-scrolling board putting a column under the corner. **The corner's 92px pad height cannot grow at ANY width**, and its width cannot grow below 1200px: at 1024 a day column's add control sits 159px above the board bottom (the corner uses 147) with its centre at x=738 (a 264px corner reaches it), and at 1280 a doubled 184px pad was measured over a day column's add control at (794,625) in the ordinary boot-tray state. Growing it buries a control on the widest profile there is. The pad height doubles to 184px in ONE place, `#sticky[data-pos="foot"] #stickyPad` at 1200px and up, because Notes has no board under it; at 1024 even that pane cannot pay (571px of content into 502px of room). The under-900px flow keeps 168px: 336 starts a scroll at 820 that was not there. Sections 13 and 16 of REVIEW.md have the measurements; tests pin all of it.
- In Notes the sticky is a right-aligned in-flow strip at the FOOT of `#main`'s column (`data-pos="foot"`, natural order, never `order:-1`), so the note list and the editor start at the top of the content area. It was above the pane until 2026-08-13 and that strip WAS the dead band over the list. Never make it an overlay in Notes: the page reaches the right edge, so a floating panel covers its toolbar, and reserving a side margin instead leaves the page ~140px wide at 1024.
...
- Behaviour suite (fast, no browser):
  ...
  node tests/test.js        # expect: RESULT: 1315 passed, 0 failed
