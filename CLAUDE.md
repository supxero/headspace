...
- FIVE panels in the rail share one near-white surface in BOTH themes: the four `.navbtn` items and `#fpanel`, all on `--northbg` = `#F6F6F7`, the app's stand-in for white (mono's brightest value, so the no-white rule needs no exception). Because a dark theme cannot lend a light panel its near-white inks, they share an ink set (`--northtxt`, `--northmut`, `--northmut2`, `--northline`, `--northfield`, `--northink-rgb`, `--northfocus`, `--northfocus-rgb`, declared in both blocks) which `#fpanel` and `.navbtn` re-point onto the standard roles. Rules inside either must keep reading the ordinary role (`--mut`, `--txt`, `--cloud`, ...) and never a `--north*` variable directly, or the island stops working. Any wash painted on these surfaces must LAYER (`background-image`), never replace `background`: as a background-color a translucent wash lets the rail show through, which under mono turns a hovered item near-black.
- The active nav item has NO surface of its own, since all five panels share one. It is told apart by four marks that must all stay: the inset 3px `--today` bar, the `--today` icon tint, the `--northline` rim, and the label stepping to `--northtxt` at weight 600 (the resting one is `--northmut` at 500). That ink step is 1.78x in Cloud blue and 1.76x in Monochrome, and a test enforces 1.5x. Do not solve an active-state problem by giving it back a different surface or by inventing a colour, border style or effect.
...
- Behaviour suite (fast, no browser):
  ...
  node tests/test.js        # expect: RESULT: 1289 passed, 0 failed
