...
State of the code as of the commit that added the habits band (2026-08-10). Written for a reviewer who has not read the file. Line numbers refer to `index.html` at this commit and will drift; section names and function names are the stable handles. This is a critical review, not a status report: the Weak points and Risk list sections are the ones to read first.

## 1. Structure

One file, `index.html`, 2734 lines: CSS to line 517, static HTML to 571, then one `<script>` to the end. No modules, no build. `sw.js` (36 lines) is a cache-first service worker keyed on a manually bumped `CACHE` constant. Tests are `tests/test.js` (1607 lines, jsdom) plus `tests/visual.py` (109 lines, Playwright, currently unrunnable: no Python on the dev machine).
...
|---|---|
| 573–589 | helpers |
| 590–605 | state globals (`state`, `ui`), `fresh()` |
| 606–745 | per-item change tracking: `eachItem`, `marksSigFor`, `sigC`/`sigD`/`sigP`, `snapSigs`, `stampChanges`, `stampLegacy` |
| 746–998 | merge: `flatten`, `pickSide`/`pickNewer`, `mergeTombs`, `stateSig`, `mergeStates`, `mergeBins`, `mergeMarks`, `carryUnknown`, `rebuild`, then `load`/`save`/`commit` and the lifecycle hooks |
| 999–1335 | Supabase sync: REST calls, `pushSoon`/`flushPush`/`putState`, `applyCloud`, `syncCycle`, 25s poll, connect/forget, sync modal |
| 1336–1443 | data ops (`findTask`, `pull`, `place`, `move`, `rollover`, `pastOpenCount`, `rollIfNewDay`), toast/undo |
| 1444–1593 | recycle bin: `binCapture`, restore/purge/empty, bin modal |
| 1594–1919 | metrics + rail, quick add, tray, board nav, board render |
| 1920–1968 | habits band: `hweek`, `renderHabits` |
| 1969–2044 | calendar render, master `render()` |
| 2045–2375 | the delegated click handler and `change` handler |
| 2376–2462 | keyboard, blur (rename commit), drag and drop |
| 2463–2657 | custom pickers, desktop pointer only: popover layer over `#jumpDate`, the per-task `pickdate` input and the `#qd` select |
| 2658–2734 | modals (help, tab-delete confirm), autogrow, resize, boot |
...
- The delegated click handler (`index.html:2064`) is ~290 lines and 69 `case`s with two exit conventions: `return` means the case handled its own save/render, `break` falls through to a shared `save(); render()`. Picking the wrong one is an easy, silent mistake, and the pre-switch block that closes the inline add field (added for the mobile trap fix) runs a deferred `save(); render()` in a `setTimeout(0)` that has to guess whether a rename just started by sniffing `document.activeElement.dataset.kind`. It works and is tested, but it is the most fragile control flow in the file.
...
state = {
  ...
  bin:    { id: { k:'task'|'sub'|'float'|'focus'|'habit', at, body, loc?, pid?, pname?, moved?, purged? } },
  ...
  settings: { view, boardOffset, floatMode, activeFloat, calSel, calOffset,
              mRange, stripDay, lastRoll, showDone, habitsOpen }
}
...
- **Merged item-by-item:** everything under `days`, `carry`, `focus`, `floats` (tasks, subtasks, tabs), `habits.list`, plus `tomb` and `bin`; `habits.marks` merges cell-by-cell as above.
- **Device-local under merge:** the entire `settings` object. `rebuild()` (`index.html:901`) takes only `mine.settings`. `VIEWSET` (`index.html:1008`) is used only by the manual-Pull path (`applyCloud`) to keep local view state while importing the rest of the remote settings. So merge and manual Pull treat settings differently; that asymmetry is deliberate but undocumented outside this file.
...
Habits band, verified in headless Chrome on 2026-08-10 with real mouse dispatch: at 1280x800 the panel sits below the day columns inside the first viewport with zero clicks, real clicks drive add, schedule edit and tick, no computed color in the panel is `rgb(255,255,255)`, no control renders blank, and Hide survives a full reload. At 390x844 the page has no horizontal overflow and the panel stacks below the day column, its grid scrolling inside itself when the tool buttons need more width than the viewport gives. Under the same touch profile a tap on the per-task date field leaves the input in place (the B fix) and opens no custom popover.
...
9. **`habits.marks` only ever grows.** Every week's cells stay forever (deliberately: history for review and future streaks), untick markers are kept as negative cells so they can outrank older ticks on late-syncing devices, and a deleted habit's marks are kept so a Restore recovers its whole record. All of that is the same never-prune stance `state.days` takes, and it feeds the same `stateSig` cost noted in Section 4. A pruning pass, if one is ever wanted, must respect the 45 day tombstone horizon or an offline device resurrects old cells.
